...
import { getAdvertisements, deleteAdvertisementById } from 'apiSdk/advertisements';
import { AdvertisementInterface } from 'interfaces/advertisement';

type ColumnType = ColumnDef<AdvertisementInterface, unknown>;

interface AdvertisementListPageProps {
...
export function AdvertisementListPage(props: AdvertisementListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<AdvertisementInterface>>(
    () => `/advertisements?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: AdvertisementInterface) => {
    if (hasAccess('advertisement', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/advertisements/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/advertisements/view/${record.id}`} passHref legacyBehavior>
                <Button
                  onClick={(e) => e.stopPropagation()}
                  mr={2}
                  padding="0rem 8px"
              ...
              {hasAccess('advertisement', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/advertisements/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('advertisement', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Box p={4} rounded="md" shadow="none">
      <Flex justifyContent="space-between" mb={4}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="advertisement" />
        </Flex>

        {hasAccess('advertisement', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/advertisements/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
