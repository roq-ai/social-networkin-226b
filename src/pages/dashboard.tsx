...
import { getAdvertisements } from 'apiSdk/advertisements';
import { AdvertisementInterface } from 'interfaces/advertisement';

import { AdvertisementListPage as ShowCaseList } from 'pages/advertisements';
...
function DashboardPage() {
  ...
  const advertisementStat = useSWR<PaginatedInterface<AdvertisementInterface>>(
    () => `/advertisements`,
    () => getAdvertisements({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; isLoading: boolean }[] = [
    ...
    },
    {
      ...
      result: advertisementStat?.data?.totalCount,
      isLoading: advertisementStat.isLoading,
    },
  ];
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`advertisements`} passHref legacyBehavior>
                  <Button
